describe('cipher', () => {

  it('debería ser un objeto', () => {
    assert.equal(typeof cipher, 'object');
  });

  describe('cipher.encode', () => {

    it('debería ser una función', () => {
      assert.equal(typeof cipher.encode, 'function');
    });

    it('debería retornar "HIJKLMNOPQRSTUVWXYZABCDEFG" para "ABCDEFGHIJKLMNOPQRSTUVWXYZ" con offset 33', () => {
      assert.equal(cipher.encode(33,'ABCDEFGHIJKLMNOPQRSTUVWXYZ'), 'HIJKLMNOPQRSTUVWXYZABCDEFG')
    });

    it('debería retornar "hijklmnopqrstuvwxyzabcdefg" para "abcdefghijklmnopqrstuvwxyz" con offset 33', () => {
      assert.equal(cipher.encode(33,'abcdefghijklmnopqrstuvwxyz'), 'hijklmnopqrstuvwxyzabcdefg')
    });

    it('debería retornar "123 ?¡@)" para "123 ?¡@)" con offset 33', () => {
      assert.equal(cipher.encode(33,'123 ?¡@)'), '123 ?¡@)')
    });
  });

  describe('cipher.decode', () => {

    it('debería ser una función', () => {
      assert.equal(typeof cipher.decode, 'function');
    });

    it('debería retornar "ABCDEFGHIJKLMNOPQRSTUVWXYZ" para "HIJKLMNOPQRSTUVWXYZABCDEFG" con offset 33', () => {
      assert.equal(cipher.decode(33, 'HIJKLMNOPQRSTUVWXYZABCDEFG'), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ')
    });

    it('debería retornar "abcdefghijklmnopqrstuvwxyz" para "hijklmnopqrstuvwxyzabcdefg" con offset 33', () => {
      assert.equal(cipher.decode(33,'hijklmnopqrstuvwxyzabcdefg'), 'abcdefghijklmnopqrstuvwxyz')
    });

    it('debería retornar "123 ?¡@)" para "123 ?¡@)" con offset 33', () => {
      assert.equal(cipher.decode(33,'123 ?¡@)'), '123 ?¡@)')
    });
  });

  describe('cipher.createCipherWithOffset', () => {

    it('debería ser una función', () => {
      assert.equal(typeof cipher.createCipherWithOffset, 'function');
    });

    it('debería retornar un objeto con dos funciones (encode y decode) con offset fijado', () => {
      assert.equal(typeof cipher.createCipherWithOffset(), 'object');
      assert.equal(typeof cipher.createCipherWithOffset().encode, 'function');
      assert.equal(typeof cipher.createCipherWithOffset().decode, 'function');
    });

    it('debería retornar "HIJhij123 ¡?@" para "ABCabc123 ¡?@" con offset 33', () => {
      assert.equal(cipher.createCipherWithOffset(33).encode('ABCabc123 ¡?@'), 'HIJhij123 ¡?@')
    });

    it('debería retornar "ABCabc123 ¡?@" para "HIJhij123 ¡?@" con offset 33', () => {
      assert.equal(cipher.createCipherWithOffset(33).decode('HIJhij123 ¡?@'), 'ABCabc123 ¡?@')
    });
  });
});